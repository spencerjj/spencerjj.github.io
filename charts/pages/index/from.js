...
Page({
  ...
  getTop(e) {
    ...
    // 区域统计
    getRequest(getApiHost(), 'platform/v1/api/minireport/lampo/findChannelSumList', 'body', data1, 0, false, false).then(
      res => {
    ...
    getRequest(getApiHost(), 'platform/v1/api/minireport/lampo/findChannelDetailList', 'body', data, 0, false, false).then(
      res => {
        if (res.result && res.result == 'login') {
          that.login()
          console.log('登录失效')
        ...
        var categoriesList = []
        for (let x in list) {
          for (let y in list[x].lampoChannelSaleList) {
            moneyList.push(list[x].lampoChannelSaleList[y].money.toFixed(0))
            categoriesList.push(list[x].lampoChannelSaleList[y].fimonth)
          }
          chartList.push({
            name: list[x].channel,
            money: moneyList,
