...
Page({
  ...
  getTop(e) {
    ...
    // 区域统计
    getRequest(getApiHost(), 'platform/v1/api/minireport/lampo/findAreaSumList', 'body', data1, 0, false, false).then(
      res => {
    ...
    getRequest(getApiHost(), 'platform/v1/api/minireport/lampo/findAreaDetailList', 'body', data, 0, false, false).then(
      res => {
        ...
        var categoriesList = []
        for(let x in list){
          for(let y in list[x].lampoAreaSaleList){
            moneyList.push(list[x].lampoAreaSaleList[y].money.toFixed(0))
            categoriesList.push(list[x].lampoAreaSaleList[y].fimonth)
          }
          chartList.push({
            name:list[x].areaname,
            money:moneyList,
